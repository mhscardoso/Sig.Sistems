...
\begin{figure}[h!]
    \includegraphics[scale=0.3]{img3a}
    \centering
\end{figure}
...
\[X_{k} = \frac{1}{4}\biggl[\frac{4j\,cos(k\pi)}{k\pi} - \biggl(\int_{-2}^{0} e^{-t}(cos(-k\frac{\pi}{2}t) + j\,sen(-k\frac{\pi}{2}t))\,dt + \int_{0}^{2} e^{t}(cos(-k\frac{\pi}{2}t) + j\,sen(-k\frac{\pi}{2}t))\,dt \biggr)\biggr]\]

\[X_{k} = \frac{1}{4}\biggl[\frac{4j\,cos(k\pi)}{k\pi} - 
\biggl(
...
\[X_{k} = \frac{2 - 2e^2cos(k\pi)}{4 + k^2\pi^2} + \frac{jcos(k\pi)}{k\pi}\]

Agora, é fácil ver que existem indeterminações para o caso em que $k = 0$. Dessa forma, com este caso já calculado, temos todos os casos para $k$.
