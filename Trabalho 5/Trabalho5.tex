...
Aqui, $u[k] = k \cdot 1[k]$. Logo, $U(z) = \frac{z}{(z - 1)^2}$

Agora, temos: $y[k] = y[k - 1] \left(1 - \frac{T}{2}\right) + Tu[k - 1]$. Aplicando a transformada Z, teremos:
\begin{align*}
    Y(z) = z^{-1}Y(z) \left(1 - \frac{T}{2}\right) + Tz^{-1}U(z)
\end{align*}
...
\begin{align*}
    ...
    x(kT) - x(kT - T) = Tw(kT - T) \\
    x(kT + T) = x(Kt) + T(A_dx(KT) + B_du(kT)) \\
    x_{k+1} = (I + TA_d)x_k + B_dTu_k \\
    y_k = C_dx_{k}
\end{align*}
