...
Como o SLIT é relaxado, podemos assumir que y[0] = 0.\\
Basta agora encontrar valores iniciais o suficiente para obter o resto.\\
Sendo $u[k] = 1[k]$, a expressão fica $y[k+2]-\frac{1}{4}y[k] = 1[k+1]+2u[k]$

\begin{align*}
    Para\ k&=-1: \\
    y[1]-\frac{1}{4}y[-1] &= 1[0]+2u[-1] \\
    y[1] &= 1 \\
    Para\ k&=0: \\
    y[2]-\frac{1}{4}y[0] &= 1[1]+2u[0] \\
    y[2] &= 3 \\
    Para\ k&=1: \\
    y[3]-\frac{1}{4}y[1] &= 1[0]+2u[-1]\\
    y[3] &= \frac{13}{4}
\end{align*}
Omitiremos os cálculos a seguir, mas o leitor é convidado a verificar a validade destes.
\begin{align*}
    y[0] &= 0\\
    y[1] &= 1\\
    y[2] &= 3\\
...
\begin{align*}
    Y(z) = \frac{z + 2}{z^2 - \frac{1}{4}}U(z)
\end{align*}
Ao dividir por z, vamos trabalhar com a seguinte expressão:
\begin{align*}
    \frac{z + 2}{z \cdot(z + \frac{1}{2}) \cdot(z - \frac{1}{2})} = \frac{r_1}{z + \frac{1}{2}} + \frac{r_2}{z - \frac{1}{2}} + \frac{r_3}{z}
\end{align*}
Após calcular, os resíduos são: $r_1 = 3;\ r_2 = 5;\ r_3 = -8$
\begin{align*}
    Y(z) &= \left( \frac{3z}{z + \frac{1}{2}} + \frac{5z}{z - \frac{1}{2}} - 8 \right) \cdot U(z)\\
    Logo:\\
    y[k] &= 3 \left(-\frac{1}{2}\right)^k \cdot 1[k] + 5 \left(\frac{1}{2}\right)^k \cdot 1[k] - 8 \cdot 1[k]
\end{align*}

(f) comparar os resultados iterativo e analítico.

Os resultados não são compatíveis dentro do intervalo de 3 vezes o desvio padrão.
